...
# --- FUNKSIONI KRYESOR QË PRONONON MESAZHET ---
@bot.message_handler(content_types=['text', 'photo', 'video'])
def handle_incoming_message(message):
    user_id = message.chat.id
    
    # KJO ËSHTË PJESA QË RREGULLON PËRSËRITJEN:
    # Nëse mesazhi është tekst dhe përputhet me butonat, MOS vazhdo më tej këtu.
    if message.text in ["Po, posto-je ✅", "Jo, kam bërë një gabim ❌"]:
        return 

    # Ruajmë mesazhin origjinal
    user_data[user_id] = message
    
    # NJOFTIMI PËR TY (ADMININ)
...
def process_confirmation(message):
    ...
    # Heqim butonat nga ekrani menjëherë
    ...
    if message.text == "Po, posto-je ✅":
        try:
            ...
            bot.send_message(user_id, "❌ Gabim: Boti duhet të jetë Admin në kanal.", reply_markup=remove_markup)
    else:
